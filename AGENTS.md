...
- [docs/wiki-part-preview.md](docs/wiki-part-preview.md) - the `apps/partpreview/` mini app: the wiki-facing embed/manifest contract (`?part_id`/`?skybox_id`/`?connectors`, `manifest.json`), how a second standalone Vite build shares the main app's assets via `assetBase()`/`VITE_ASSET_BASE`, and how to add another mini app
