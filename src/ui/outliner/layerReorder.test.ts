...
// The document order a fresh project has: Default, the three pinned entity-only layers, then
// whatever the user added. Pinned ids sit BETWEEN ordinary ones, which is the whole reason
// the ordinary partition has to be lifted out and put back rather than sliced.
const IDS = ['default', 'ivaSeats', 'lights', 'kittens', 'layer1', 'layer2'];
...
describe('withOrdinaryOrder', () => {
  it('keeps every pinned layer at its document position', () => {
    expect(withOrdinaryOrder(IDS, ['layer2', 'layer1', 'default'])).toEqual([
      'layer2',
      'ivaSeats',
      'lights',
      'kittens',
  ...
  it('is a permutation of the input', () => {
    const out = withOrdinaryOrder(IDS, ['layer1', 'default', 'layer2']);
    expect([...out].sort()).toEqual([...IDS].sort());
  });
});

describe('ordinaryIds', () => {
  it('drops the pinned entity-only layers', () => {
    expect(ordinaryIds(IDS)).toEqual(['default', 'layer1', 'layer2']);
  });
});

describe('movedOrdinary', () => {
  it('swaps with the ordinary neighbour, skipping the pinned layers between them', () => {
    expect(movedOrdinary(IDS, 'layer1', -1)).toEqual([
      'layer1',
      'ivaSeats',
      'lights',
      'kittens',
  ...
  it('returns null for a pinned or unknown layer', () => {
    expect(movedOrdinary(IDS, 'lights', 1)).toBeNull();
