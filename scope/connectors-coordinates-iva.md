...
| `src/state/ivaStore.ts` / `ivaSeatStore.ts`         | Ephemeral, never persisted, never in undo. `$seatView` (keyed by seat **id**, so a mid-preview reorder can't move the camera into another seat) + `$seatLook` (the current unit **look DIRECTION**, mirroring KSA's own state — see the `Viewport` row) + `enterSeatView`/`exitSeatView`/`nudgeSeatLook`; `$ivaSeatAimRequest` is the "Aim at selection" intent `EditorScene` consumes (the `$colliderFitRequest` pattern) before writing through `aimIvaSeat`.                                                                                                                                                                                                       |
| `src/three/Viewport.ts` / `EditorScene.ts`          | The seat-view camera mode: `enterSeatView(pose)` snapshots the orbit camera, disables `OrbitControls` **and skips `controls.update()`**, and per change applies the pointer delta to the STORED look direction → `clampSeatLook` (ONCE) → store → `lookAt`. **The state must be the direction, not a yaw/pitch accumulator**: KSA clamps its own previously-clamped look every frame (`IVAController.OnFrame`) and therefore converges; re-composing from a raw accumulator makes each single pass under-correct, and the preview measurably escapes BOTH clamps (`dot(look, forward)` reached −0.23, `                                                               | dot(look, up) | `reached 0.902). A degenerate`cross(look, up)` leaves the orientation untouched rather than letting three.js invent a roll. No FOV change — flexo's camera is already KSA's **50°** (`Camera.cs:51`). `EditorScene` resolves the id to a pose, exits cleanly on a vanished seat, and suppresses the gizmo, click-selection and the seat markers while seated. |
...
| Seat authoring UI                                   | `ui/build/SeatInspector.tsx` (order + "IVA opens on this seat" badge, the **exported-axis readout** through `formatG6`, six aim presets that KEEP the current up axis where it stays non-parallel, "Aim at selection", "Sit in this seat", "Add kitten at this seat"); the Outliner's IVA Seats layer (rows are ORDINALS — a seat has no name — subtitled with the derived forward axis); **Add → IVA Seat**; the status bar's tool segment (`ui/status/ToolSegment`: prev/next wrap like `C`, Exit; `Escape` is rung 8 of `ui/hotkeys/escLadder`, gated on `$seatView` and never `preventDefault`ed so it can't shadow a dialog dismiss).                            |
...
- **A connector's `<Scale>` is LOAD-BEARING game data, not marker size.** `Part.Connector` copies `Transform.ScaleValue` into `Connector.Scale` and compares `Scale.LengthSquared()` BETWEEN connectors to resolve `Internal` mating and the aligned/nested-connector search (`Part.cs` ~L215-366) — i.e. it is an attach-node **size class**. flexo therefore round-trips it verbatim, lets it be edited directly (single-select gizmo/inspector), and **excludes it from every group/global resize**: a multi-select scale or "Scale Everything" MOVES a connector but never re-grades it (`scalesWithGroup` in `src/three/bulkTransform.ts`, `scaleEverything` in `editorStore.ts`). Colliders, whose `scale` IS meters of geometry, do scale.
...
re-homed the three that survive onto `Camera` as `ForwardView`/`RightView`/`UpView`, with
**identical values** — `-double3.UnitZ` / `+double3.UnitX` / `+double3.UnitY`, exactly what
`Double3Ex` returned. `Camera.GetForward()`/`GetRight()`/`GetUp()` became
`GetForwardEcl()`/`GetRightEcl()`/`GetUpEcl()` (same bodies, clearer frame naming), and
`QuaternionEx.GetAxis`'s degenerate fallback went from `Double3Ex.Up` to the identical
`double3.UnitY`. `QuaternionEx.CreateFromXyzRadians` is byte-identical. **No recalibration:**
`coords.ts`'s `EULER_ORDER` and `ivaSeatAxes.ts` are untouched, and `ivaSeatAxes.test.ts` — the
canary — passes. When quoting the axis convention, cite `Camera.ForwardView` et al. from 5117
onward; `Double3Ex` no longer has them.

**`ivaLook.ts` port — intact.** `IVAController.OnFrame` changed only by following that rename
(`Camera.GetUp()` → `GetUpEcl()`, `Double3Ex.Forward` → `KSA.Camera.ForwardView`). Both view
clamps, the `0.9` up-pole test and the seat-cycling / `OnSwitchOn` order semantics are unchanged.
`IVASeatTemplate`'s three `Vector3Reference` fields and their defaults are byte-identical.

**Seats and hatches — the crew-assignment feature (rev 5083/5085) reached the template schema.**

|                   | 5056                                                  | 5117                                                                                             |
| ----------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `EVADoorTemplate` | no fields at all                                      | `[XmlAttribute("SeatId")] string SeatId = ""`                                                    |
| `<IVASeat Id>`    | schema-legal, authored by nobody                      | authored by Core, resolved by `EVADoor.ResolveAlignedSeats`                                      |
| Boarding          | "Enter vehicle" takes the first free seat, 15 m range | targets the door's **aligned** seat, 25 m range, and the button hides when that seat is occupied |
| EVA               | spawns a fresh `Kitten_N`                             | spawns the kitten assigned to the aligned seat (`KittenRoster`)                                  |
...
5117 decomp returns only Vulkan tessellation state and the spline editor — so "up follows the
root part" still holds, `FlightComputer.UpdateAttitudeTrackError` still aims body **+X**, and
`VehicleEditor` still pins the root to identity. `PartModelModule`'s `[XmlElement("Internal")]`
is still the only one in the tree (`<PartModelGlass>` gained none). `Part.Connector`'s `Flag`
enum and `<Flags>` schema are unchanged; `ConnectAndMerge` only gained an `out PoseChange` for
the new docking-camera smoothing.
