...
- ✅ **`<Control/>` (was MISSING-CAPABILITY — FIXED).** New empty command marker on command pods; flexo now models it as `PartGameData.controllable` (parse = presence of a `<Control>` child; emit = a bare `<Control/>`), carried through catalog/import/persist so a re-exported capsule stays controllable.
