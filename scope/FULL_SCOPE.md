...
|                      | Build            | Path                                                                                                                                                               |
| -------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Verified against** | `2026.9.7.5402`  | `ksa-game-assemblies/current` (decomp @ 5402) + `flexo-private-assets/assets` (Core XML @ 5402, UASTC re-encoded); also `ksa-game-assemblies` git commit `57e6040` |
| Previous baseline    | `2026.8.22.5348` | `ksa-game-assemblies_prev/current`, also git commit `c465abb` (the mirror \_prev copies go stale over time — diff the decomp via git history)                      |
| Baseline before that | `2026.8.19.5261` | `ksa-game-assemblies` git commit `1401af7` (diff decomp via git history)                                                                                           |

> ✅ **No unvetted build outstanding.** `ksa-game-assemblies/current` and the private asset mirror
> are both at **`2026.9.7.5402`**, and that is the build this catalog is now verified against.
> The mirror had already been advanced to 5402 and UASTC-transcoded (`flexo-private-assets`
> commits `3825724` + `b1082c1`), so no further sync or re-encode pass was needed, and all ten
> vendored `src/ksa/__fixtures__/` files are byte-identical to 5402 — no fixture re-sync either.
...
> **5402 review method.** Full `5348 → 5402` diff via `diff -rq` of the two provided asset trees
> (`ksa-game-assemblies_prev/current` @ 5348 vs `ksa-game-assemblies/current` @ 5402): **216
> changed + 66 added + 0 removed `decomp/*.cs`** (after the Brutal/System/MIConvexHull/Planet
> filter) and 19 changed + 3 added `Content/` files (`ParachuteAssets.xml`,
> `Shaders/Mesh/StaticObjectNormalIndirect.frag`, `Shaders/VolumetricExhaust/PlumeBend.glsl`); a
> sweep of every changed and new `.cs` for `[XmlElement]`/`[XmlAttribute]`/`[XmlType]`/`[XmlIgnore]`/
> `[DefaultValue]` hunks; and byte-identity for every ported class by construction (none is in the
> diff). `version.json` @5402 is thin — `fromRevision: 5400`, one documented commit (rev 5401,
> "Fixed crash for incorrect data stride for thumbnail rendering"); revs 5349–5400 have no changelog,
> so the file diff is the authority. The churn is a `Viewport` → `IViewport`/`IGameViewport`
> multi-viewport refactor (new `PartThumbnailViewport`, `ViewportRegistry`, …), KSA's **parachute**
> system (~30 `Chute*`/`Parachute*` classes on a Bepu cloth solver), a **part structural-failure**
> model (`PartStructuralLimits` / `PartFailure` / `PartContactLoad`) and a plume-trail rework — whose
> **entire XML schema delta is 5 declarations**, plus one new content _pattern_.
>
> Findings: two **MISSING-CAPABILITY**, both fixed. (**U1**) `PartTemplate` gained
> `[XmlAttribute] double CrashTolerance = NaN`, a **geometry-`<Part>` root attribute** that
> `ApplyGameData` never copies (so the GameData passthrough could not save it); it feeds
> `PartStructuralLimits.ResolveCrashTolerance` (authored Pa, else derived from inert mass ÷
> bounding-box volume) and `PartFailure.Detect`. Core authors `CrashTolerance="3e6"` on the five
> CorePropulsionA engines, which flexo dropped on round-trip. Now `PartGameData.crashTolerancePa`,
> parsed off and emitted on the `<Part>` element, imported, persisted (`crt`, additive — no schema
> bump) and authorable in Identity. (**U4**) `CoreUtilityAGameData.xml` is the first Core file to
> author geometry `<SubPart>` **templates** inside a _GameData_ document
> (`CoreUtilityA_Subpart_StandaloneParachuteA/B`, resolving mesh + material ids from the sibling
> Assets file) and to **add** placements from `<PartGameData>` (`ParachuteBayB`'s two packed chutes,
> via `PartTemplate.ApplyOrAddSubPartInstance`); flexo's catalog scanned only `ASSET_FILES` and
> `mergeGameData` dropped GameData-added placements (`<SubPart>` is a modeled child, read for
> `<Gimbal>` only), so the bay imported without its chutes. `parseAssetsFile` now takes the sibling
> GameData doc and `mergeGameData` appends unmatched `<SubPart InstanceOf>` entries.
>
> One **MISSING-CAPABILITY, open**: the new `<Parachute>` `Components` module (**U2** — 15
> attributes + `<Attach X Y Z>`; Core authors it under both `<PartGameData>` and
> `<SubPartGameData>`, where the passthrough round-trips it verbatim but the editor cannot author
> it). One **NONE / low, open**: `PartTemplate.SubPartGroups` (`<SubPartGroup><SubPartRef Id>`,
> **U3**) has no consumer in the decomp and no Core author; passthrough under GameData, dropped on a
> geometry `<Part>` like S2/T3. One **COSMETIC, fixed**: `PlumeTrailTemplate` gained `<Color>` /
> `DensityMultiplier` / `<Lifetime>` and Core added a second template, `LiquidEnginePlumeTrail`
> (unreferenced so far) — flexo uses trails by id only, and its static `PLUME_TRAIL_IDS` snapshot
> was one entry short (**U5**). `CharacterCoreReference.HeadMeshIndices` (the fifth declaration)
> is a first-person head hide (`KittenRenderable.HideHead`, set by `IVASeat`) — NONE for the aide.
>
> Explicitly **re-verified intact**: every verbatim-ported engine class, the reaction family +
> `Reactions.xml`, the solid-motor + grain-geometry set, every unit reference, `KeyframeAnimationData`
> / `KeyframeAnimationModule` (`Skeleton.cs` only gained `CloneRig()`), `Double3Ex`,
> **`ThumbnailRenderResources` byte-identical despite rev 5401** (the stride fix landed in the
> viewport plumbing — synthetic Normal + ORM stay mandatory), `PbrMaterialReference` (T1 still open),
> `PartModelModule` (only the `IsLightSwitchedOff()` refactor; `<Internal>` still unique),
> `IVASeatTemplate` / `EVADoorTemplate` / `DockingPortTemplate` / `BatteryTemplate`,
> `Lighting/Light.cs` + both light shaders, `ConsumerFeedWiring` / `FeedsFromReference` /
> `RocketCoreTemplate` / `ConnectorCapability*`, `SubPartTemplate` / `EditorTagDefinition`,
> `Control` / `ControlTemplate` (`FlightComputer` viewport-only, still body **+X**), `Mod` /
> `AssetBundle` / `ModManifest` (`ModLibrary` viewport-only → multi-part bundle contract unmoved),
> all collider classes, `GltfUtils`, the static-object + launch-site template classes,
> `QuaternionEx` (a `Normalize` decompiler shape only; `CreateFromXyzRadians` untouched),
> `IVAController` (default ctor name + one cursor line; both view clamps intact), `IVASeat` (only
> `HideHead`), and `ENABLE_EMISSIVE` still defined in `PartModelRenderer` and absent from the
> dynamic pipeline. `ModelPbr.frag` / `ModelNormal.frag` flip the geometric normal on back faces
> for the two-sided canopy; `getNormalFromMap_ShaderX` is unchanged. `ConstraintSim` now collides
> against **every** launch-pad landmark on a body (a `LaunchPadPose` list) instead of the nearest
> within 300 m, and `StaticObjectRenderer` loads a dedicated `StaticObjectPrePassIndirectFrag` —
> both noted in the ICRP scope docs.
>
> Content-side: the parachute bay was re-cut as `CoreUtilityA_Prefab_ParachuteBayB` (14 new
> SubParts with box/cylinder colliders) plus four radial chute Parts tagged `Radial` + `Landing`
...
Status reflects the `5348 → 5402` review. 🔴 breaking · 🟡 missing/drift · 📝 docs · ✅ intact.
Rows whose contract did not move at 5402 keep their prior verdict and are marked "re-verified".

**5402 deltas to the table below:** _Part/SubPart XML_ → 🟡 **fixed in this review** twice over
(`<Part CrashTolerance>`, a geometry-root attribute `ApplyGameData` never merges — gap **U1**; and
Core's first GameData-file SubPart templates + GameData-**added** placements, which the catalog now
scans and `mergeGameData` now appends — gap **U4**), and widens by one passthrough-only element
(`<SubPartGroup>`, no consumer yet — gap **U3**). _GameData modules_ → 🟡 (the new `<Parachute>`
`Components` module is passthrough-safe at both authoring sites but opaque to the editor — gap
**U2**). _Engines / Nozzle exhaust_ → ✅ with a COSMETIC snapshot refresh (`PlumeTrailTemplate`
gained `<Color>`/`DensityMultiplier`/`<Lifetime>`, Core added `LiquidEnginePlumeTrail`;
`PLUME_TRAIL_IDS` refreshed — gap **U5**, fixed). _Kittens_ stays 📝 on **T4**
(`<HeadMeshIndices>` is a first-person head hide the aide never needs). _Custom assets_ stays 🟡 on
**T1** (`ThumbnailRenderResources` byte-identical despite rev 5401's thumbnail fix). _Static
objects_ / _Launch sites_ ✅ re-verified (schema identical; `ConstraintSim` now collides against
every launch pad on the body; a dedicated static-object pre-pass shader). Every other row
re-verified ✅ against 5402.

**Superseded 5348 deltas:** _GameData modules_ → 🟡 **fixed in this review** (rev 5329's
...
### Open gaps from 5402 → [plans/FIX_CURRENT_GAPS_PLAN.md](../plans/FIX_CURRENT_GAPS_PLAN.md)

The `5348 → 5402` review found **no BREAKING item**. Two MISSING-CAPABILITY gaps that bit live Core
data were fixed in the review itself, one MISSING-CAPABILITY and one low-severity item remain 📋
OPEN, and one COSMETIC snapshot was refreshed:

- **U1 — `<Part CrashTolerance>` (🟡 MISSING-CAPABILITY) — ✅ FIXED.** `PartTemplate` gained
  `[XmlAttribute] double CrashTolerance = double.NaN` for the new part structural-failure model
  (`Part.CrashTolerancePascals` → `PartStructuralLimits.ResolveCrashTolerance`; NaN/≤ 0 ⇒ derived
  from inert mass ÷ bounding-box volume, clamped 0.1–20 MPa; `PartFailure.Detect` sheds a part
  whose contact pressure reaches it). It is a **geometry-`<Part>` root attribute** that
  `PartTemplate.ApplyGameData` never copies, so neither `<PartGameData>` authoring nor the
  root-attribute passthrough could carry it, and Core's `CrashTolerance="3e6"` on
  `CorePropulsionA_Prefab_EngineA2…A6` was dropped on round-trip. Now `PartGameData.crashTolerancePa`
  (`src/ksa/types.ts`), read by `crashToleranceFromPartElement` and emitted on `<Part>` by
  `serializePartsXml`, carried through the catalog/import/paste/transfer paths, persisted as the
  additive codec key `crt` (no `PROJECT_SCHEMA_VERSION` bump) and authorable in Identity.
- **U2 — `<Parachute>` is opaque to the editor (🟡 MISSING-CAPABILITY) — 📋 OPEN.** The new
  `Parachute.TemplateData` `Components` module (15 attributes + `<Attach X Y Z>`) is authored by
  Core under `<PartGameData>` (four radial chutes) and `<SubPartGameData>` (the bay's packed
  chutes); both sites ride the `RawXmlNode` passthrough, so nothing is lost today, but flexo cannot
  author a chute or show its attach point.
- **U3 — `<SubPartGroup>` (NONE today) — 📋 OPEN, low.** `PartTemplate.SubPartGroups`
  (`<SubPartGroup><SubPartRef Id/></SubPartGroup>`) is declared and merged but read by nothing else
  in the decomp, and Core authors none; passthrough under GameData, dropped on a geometry `<Part>`
  (the S2/T3 pattern).
- **U4 — GameData-file SubPart templates and GameData-added placements (🟡 MISSING-CAPABILITY) —
  ✅ FIXED.** `CoreUtilityAGameData.xml` declares two geometry `<SubPart>` templates
  (`StandaloneParachuteA/B`, mesh + material ids from the sibling Assets file) and `ParachuteBayB`'s
  `<PartGameData>` **adds** their placements (`PartTemplate.ApplyOrAddSubPartInstance`). flexo's
  catalog scanned only `ASSET_FILES` and `mergeGameData` dropped GameData-added placements (a
  modeled `<SubPart>` child, read only for `<Gimbal>`), so the bay imported without its chutes.
  `parseAssetsFile` now takes the GameData sibling; `mergeGameData` appends unmatched
  `<SubPart InstanceOf>` entries before scoping SubPartGameData.
- **U5 — `PLUME_TRAIL_IDS` one entry short (COSMETIC) — ✅ FIXED.** `PlumeTrailTemplate` gained
  `<Color>` / `DensityMultiplier` / `<Lifetime>` and `PlumeTrailAssets.xml` a second template,
  `LiquidEnginePlumeTrail` (no Core nozzle references it yet); the static snapshot in
  `src/ksa/types.ts` now lists both.
