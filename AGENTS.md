...
- [docs/projects.md](docs/projects.md) - project-based workspace persistence: a project (document + layer view state + active layer + camera + editor aids + undo/redo history) is keyed by a stable id in the `flexo-projects` IndexedDB database, autosaved on two debounces and restored on boot (awaited) before render; the boot purges, multi-tab write lock, per-project asset namespacing, thumbnails, and create/open/duplicate/rename/delete
