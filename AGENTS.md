...
- `apps/` - standalone **mini apps**, each its own Vite root/bundle built into `dist/apps/<name>/` and sharing the main app's static assets (`apps/partpreview/` = the wiki-embeddable single-Part 3D preview; `apps/icrp/` = **ICRP**, the KSA static-object / launch-complex editor — see [plans/ICRP_PLAN.md](plans/ICRP_PLAN.md), [scope/static-objects.md](scope/static-objects.md), [scope/launch-sites.md](scope/launch-sites.md), and `apps/icrp/SHARED_IMPORTS.md` for the flexo-module reuse manifest). See [docs/wiki-part-preview.md](docs/wiki-part-preview.md).
...
- [docs/action-chains.md](docs/action-chains.md) - the action-chain floating window (`⇧⌘K`; `⌘K` is now the command palette): composable transform/array steps over frozen SubPart or same-owner collider selections, collider Circular Arrangement (opening diameter, selection-centered hole, total count, default Y axle), the other op semantics (iterated linear delta, radial angle-step rule and +X default axis), the instance/ghost caps, and the one-undo-step commit (owner/layer preservation, no reference remapping, fresh ids)
...
- **Latest KSA audit:** [plans/KSA_5438_REVIEW.md](plans/KSA_5438_REVIEW.md) covers `5402 → 5438`, with the complete changed-file routing in [plans/KSA_5438_FILE_AUDIT.md](plans/KSA_5438_FILE_AUDIT.md). Use the current `scope/` verdicts; older review prose can contain corrected findings.
...
[analysis/KSA_ENGINE_DETAILS.md](analysis/KSA_ENGINE_DETAILS.md), and the current
[field/UI coverage audit](plans/ENGINE_DATA_COVERAGE.md). Controllers are modeled at both
Part and SubPart scope. `src/ui/engine/solidCurveResolution.ts` resolves the selected rocket's
solid stack using owner-relative references and parent wiring; `SolidThrustCurveCard.tsx`
plots thrust, pressure and Isp. Connector-fed vehicle stacks remain outside single-Part preview.
