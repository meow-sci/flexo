...
**Baseline status:** 🟡 **INTACT, one new slot unmodeled** — 5348 added
`PbrMaterialReference.AlphaMap` (`<Alpha>`, gap **T1**; part rendering still binds only five
textures, so nothing is lost on a part path today — see
[What changed in 5348](#what-changed-in-5348)). Otherwise ✅ (re-verified @5117 —
`ThumbnailRenderResources` byte-identical,
