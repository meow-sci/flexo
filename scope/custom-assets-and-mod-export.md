...
1. **GLB mesh-name from node.** KSA reads the SubPart id from glTF `meshes[i].Name`, but `THREE.GLTFExporter` writes only the *node* name → `null.StartsWith('_')` NRE. `nameMeshesFromNodes()` copies node→mesh name. *(`MeshAtlasFileReference.DoLoad`)*
