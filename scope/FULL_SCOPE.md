...
|                      | Build            | Path                                                                                                                                                               |
| -------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Verified against** | `2026.8.5.5168`  | `ksa-game-assemblies/current` (decomp @ 5168) + `flexo-private-assets/assets` (Core XML @ 5168, UASTC re-encoded); also `ksa-game-assemblies` git commit `c9c5221` |
| Previous baseline    | `2026.8.3.5117`  | `ksa-game-assemblies_prev/current`, also git commit `2369a41` (the mirror \_prev copies go stale over time — diff the decomp via git history)                      |
| Baseline before that | `2026.7.10.5056` | `ksa-game-assemblies` git commit `13595c1` (diff decomp via git history)                                                                                           |

> ✅ **No unvetted build outstanding.** `ksa-game-assemblies/current` and the private asset mirror
> are both at **`2026.8.5.5168`**, and that is the build this catalog is now verified against —
> the `5117 → 5168` review below covered all three areas the previous warning flagged. `git fetch`
> on `ksa-game-assemblies` reports nothing newer.
...
Status reflects the `5117 → 5168` review. 🔴 breaking · 🟡 missing/drift · 📝 docs · ✅ intact.
Rows whose contract did not move at 5168 keep their prior verdict and are marked "re-verified".

**5168 deltas to the table below:** _Ground clutter_ → 🔴 (revs 5136–5138/5157 moved clutter onto
...
| Area                                                                                      | Detail doc                                                         | Primary game anchors                                                                                                                                                                                                                                                                                                         | 5168 status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Part / SubPart XML structure, catalog, editor tags, part size                             | [part-and-subpart-xml.md](part-and-subpart-xml.md)                 | `PartTemplate.cs`, `Part.cs`, `EditorTagDefinition.cs`, `*Assets.xml`/`*GameData.xml`, `CoreEditorTagsGameData.xml`                                                                                                                                                                                                          | ✅ re-verified @5168 — structure unchanged (`PartTemplate` gained only the passthrough-covered `<SolidOgiveMass>`/`<HollowOpenOgiveMass>`); rev 5161's new `CoreUtilityAAssets.xml` was missing from `ASSET_FILES` and is now listed, with a live-tree drift guard in `catalog.test.ts`; `PartGameData.xml` fixture re-synced (values only)                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| Ground clutter (data-only celestial mod)                                                  | [ground-clutter.md](ground-clutter.md)                             | `GroundClutterReference.cs` + 6 sibling schema classes                                                                                                                                                                                                                                                                       | 🔴 @5168 — clutter moved onto the asset bundler (revs 5136–5138, 5157): `ClutterObjectTemplate` is now a top-level id-referenced asset (`Atlas=`, `<Colliders>`, exactly 5 `<LOD>`s), `<LOD><Mesh>` is a `SerializedReference` id list, and the ecotype `<Material>` list is `[XmlIgnore]`/derived. `build-cartoon-moon.ts` emits the old form and **no longer loads** (gap **R1**). Scaffold-only — no editor code involved                                                                                                                                                                                                                                                                                                                                     |
