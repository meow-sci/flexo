...
The mesh is created *and placed* at the origin in one action, and is selected. Its instance id
...
> **BulkFluid** here (a Bulk-plumbed combustor needs that capability at *both* ends).
...
3. Click the **Tanks (feed containers)** section *header* to expand it. It starts collapsed
   because the section is empty.
4. Click **+ Tank**.
5. **Feed id** → `fuel_main`  ← this is the name the feed wiring will reference.
6. **Length (m)** → `2.5`, **Outer Radius (m)** → `0.6`.
7. Leave **Shape** `Cylindrical`, **Wall Material Id** `Aluminum.2014(s)`, **Role affinity**
   `Engine (default)`.

> The feed id must be non-blank. A blank id is not addressable and the feed will not resolve.
>
> The section header also has a **＋** that adds a tank without expanding — so the new tank is
> invisible until you expand the section anyway. Expand first.

## Step 7 — Define the engine

1. Press `4` (Engine mode).
2. In the Engine navigator header, click **＋** (`Define new engine`).
3. Choose **Liquid rocket** — *combustor + De Laval nozzle + rocket + controller*.
...
> *unnormalized*, so a length of 2 silently doubles your thrust. If you edit it, use the
...
2. Click that row. It creates the wiring entry *and* opens it in the left editor.
...
While the entry is empty it reads **`ThrustChamber ← nothing`**. That caption is the reliable
signal — see the warning below.

> **Do not stop at step 2.** A wiring entry with **no feed points** silences the "no wiring"
> warning while delivering nothing — and flexo *omits it from the export entirely*. Verified:
> the Issues panel reads `Issues — no issues` both with the feed set **and** with the entry
> empty. Your only signals are the red *"No feed points — KSA will log 'wires no feed points'"*
> note in the Feeds-from box, and the `← nothing` caption on the tree row.
>
...
1. In the module tree, click **＋** on the **Gimbals** group header.
   - With one candidate placement the button reads *"Add a gimbal on `flexo_bell_…_1`"* and
     adds it immediately.
...
> A gimbal is keyed to a **placement**, and it vectors *every nozzle on that SubPart*. A 0°/0°
...
1. Arm the placement tool, either way:
   ...
   - the **⌖ Place this nozzle's exhaust in 3D** button at the bottom of the nozzle editor,
     which targets *this* nozzle specifically and works from Data mode too.
...
- **Performance** — should read a real **Thrust (vacuum)**, **Thrust (sea level)**, **Isp**,
  **Mass flow** and **Throat diameter**. With the numbers above you get roughly **289 kN vac**.
  If it says *"Add a combustor and a nozzle to see live thrust and Isp"*, the rocket is not
  bound to both.
...
> **`Issues — no issues` does not mean the engine is fed.** The validator only checks that a
> `<ConsumerFeedWiring>` entry *exists* for a parent-deferring consumer, not that the entry has
> any feed points. An empty entry passes validation, is dropped from the export, and leaves the
> engine dry. Re-open the wiring entry and confirm **Feeds from** lists
> **Container (tank / grain) → fuel_main**. The Performance card is no help either — it reads
> chamber and nozzle geometry, and shows full thrust for an engine with no propellant path.
...
- **Solid motor** — Step 7, choose **Solid motor**. You get a real `<SolidMotor>` +
  ...
  segment *is* the tank, so skip Step 6. A solid motor needs a `Solid`-category reaction
