...
/**
 * "Export to KSA" — root-hosted by `DialogRoot` under the dialog id `'export-ksa'`.
 * Two modes:
 *   - XML: shows the raw Part / GameData / Assets KSA XML with copy-to-clipboard.
 *   - Mod: writes a KSA part mod (mod.toml + per-project XML) either into a
 *     user-granted `…/mods` folder or as a downloadable zip.
 *
 * Behaviour is v1 verbatim (pre-flight severity boxes, non-blocking export policy,
 * non-overwrite folder writes, zip fallback). The export redesign owns any change here.
 */
  ...
  return (
    ...
    >
      <Dialog className="min-h-0 flex-1">
        <DialogHeader title="Export" onClose={() => onOpenChange(false)} />
