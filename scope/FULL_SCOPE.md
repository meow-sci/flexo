...
> and a removal. See [the upgrade report](../plans/KSA_5438_REVIEW.md) and
> [the per-file audit](../plans/KSA_5438_FILE_AUDIT.md) for evidence and validation results.
> The private mirror was already updated to 5438: all shared XML matches apart from eight
> line-ending differences, and its 70 non-character KTX2 atlases are already UASTC. No private
> asset rewrite was needed. Vendored fixtures were refreshed and the current Utility GameData
> was added for parachute material-choice regressions.
...
> for the two-sided canopy; `getNormalFromMap_ShaderX` is unchanged. `ConstraintSim` caches all launch-pad landmarks on a body, but still selects only the nearest
> within 300 m for collision (the previous review incorrectly equated the candidate list with
> simultaneous collision), and `StaticObjectRenderer` loads a dedicated `StaticObjectPrePassIndirectFrag` —
...
objects_ / _Launch sites_ ✅ re-verified (schema identical; `ConstraintSim` caches every launch pad but still selects the nearest
within 300 m for collision; a dedicated static-object pre-pass shader). Every other row
...
### Historical gaps from 5402 → [plans/FIX_CURRENT_GAPS_PLAN.md](../plans/FIX_CURRENT_GAPS_PLAN.md)
