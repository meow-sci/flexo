...
| `src/ksa/exportGlb.ts`                                  | `buildMeshAtlasGlb(nodes)` — geometry-only GLB atlas; render mesh + paired `_VM` view mesh per SubPart; **`nameMeshesFromNodes()`** copies node name → `meshes[i].name`; **`toKsaGeometry()`** forces an index buffer + strips unread attributes.    |
