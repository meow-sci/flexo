...
**Engine/LES audit @5402:** `CorePropulsionAGameData.xml` authors TWO `<Decoupler Id>`
components on both `CorePropulsionA_Prefab_LESA` and
`CorePropulsionA_Prefab_LESSegmentThrustAssemblyA`. The earlier single-field assessment missed
real data loss. `Decoupler.ksaId` now preserves the first component's inherited module `Id`;
additional decouplers are retained in `unknownChildren`, including their `Id`, `ConnectorId`
and `Force`. Import/paste rewrites their `ConnectorId` with the normal connector-id map.
The Coupling editor still edits the first component only. Evidence:
`decomp/KSA/Decoupler.cs` (`TemplateData`, `CreateComponents`),
`decomp/KSA/ModuleBase.cs` (`TemplateDataBase.Id`), and
`decomp/KSA/PartTemplate.cs` (`Components.AddRange` in `ApplyGameData`). Portable full-bundle
regressions require both `NoseconeMount` and `SkirtMount` with valid exported connector ids.
...
| `Decoupler`                | `Decoupler.cs` (`TemplateData`, inherited `ModuleBase.TemplateDataBase.Id`)         | `<Decoupler>`                                                                       | attrs `ConnectorId` (string), `Force` (float N)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | **unchanged**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
segments. The later 5402 engine audit found that the repeatable decoupler case was missed;
it now preserves named primary and additional decouplers as described above.
...
`<PartGameData>`. Its inherited `Id` and repeatability still need preservation in flexo,
corrected in the 5402 engine audit above. Two behavioural consequences to know:
...
## Engine field audit at 5402

`PartTemplate` accepts `<RocketEngineController>` and `<RocketThrusterController>` at both
GameData scopes. Both now have typed model/editor/XML support; old saved passthrough nodes
remain preserved without conversion. Project paste also retains SubPart unknown attributes
and children and remaps `<RocketReference SubPartId>` alongside connector references.

`SolidGrainSegmentTemplate : AsmbVolumetricMassTemplate : AsmbTransformTemplate` exposes
`<Grain><Mass>`, `<Density>`, `<Material Id>`, `<LocationAsmb X Y Z>`, and
`<Paf2Asmb X Y Z>` in addition to dimensions. The previously missing mass, density, and
rotation fields are modeled and editable, with additive project defaults. See
[engine coverage](../plans/ENGINE_DATA_COVERAGE.md) for every engine module field.
