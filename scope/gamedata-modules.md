...
**Baseline status:** 🟡 **DRIFTED (one attribute)** — 5117 added `<EVADoor SeatId>`, which flexo
drops (gap **Q1**, see [What changed in 5117](#what-changed-in-5117)); every unit token, scale
factor and module element form is otherwise byte-identical.
...
| `EvaDoor`                  | `EVADoorTemplate.cs`                                                                | `<EVADoor>`                                                                         | attr `ConnectorId` — ⚠️ 5117 added attr `SeatId`, NOT modeled (gap **Q1**)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | unchanged                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
**One added attribute: `<EVADoor SeatId>` (🟡 MISSING-CAPABILITY, gap Q1). Everything else INTACT.**
...
hatch boards. Core authors it on both capsule crew doors (`PartGameData.xml`:
`<EVADoor SeatId="CoreIVASpaceA_Prefab_MediumCapsuleA_SeatA" />`). Because `<EVADoor>` is a
**modeled** child, the GameData passthrough does not cover its attributes: flexo's `EvaDoor`
carries only `connectorId` (`src/ksa/types.ts`), the parser reads only `ConnectorId`
(`src/ksa/partXmlParser.ts` ~`:674`) and the serializer writes only `ConnectorId`
(`src/ksa/partXmlSerializer.ts` ~`:257`), so the attribute is dropped on round-trip. Fix (no
migration): add `seatId: string` to `EvaDoor`, parse/emit it, omit the attribute when empty.
The paired half — emitting `<IVASeat Id>` — lives in
[connectors-coordinates-iva.md](connectors-coordinates-iva.md#what-changed-in-5117) as gap **Q2**.
...
triggers a `flexo_<base>_<id>` export variant (`modExport.hasSubPartGameData`) so the shared Core
template is never redefined.
