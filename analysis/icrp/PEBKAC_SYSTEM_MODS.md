...
3. If not found → a fresh `<{BODY_TYPE} Id Parent>` element is built (`:16-21,150-159`) with
   ...
   **Important consequence:** bodies referenced only as `Parent` but not selected are _not_ auto-added
...
every `Path="…"` to `Path="../Core/…"` and every `Id` that _starts with_ `"Texture"` to `"../Core/Texture…"`
...
never copies textures. It is a blunt regex-free string prefix: it does _not_ touch `MeshCollection Id`,
...
note this is the _browser_ `Document` constructor; in Node it needs jsdom/xmldom, which the
...
3. `fixPathsToCore` (`src/ts/xml/fixPathsToCore.ts`) as-is, **but scoped**: apply only to the cloned
   Core subtree, _before_ ICRP appends its own `<Landmark>`/`<Modifier Type="Decal">` nodes whose
