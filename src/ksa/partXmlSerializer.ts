...
function appendPartGameData(
  ...
  for (const seg of game.solidGrainSegments)
    gd.appendChild(buildSolidGrainSegmentElement(doc, seg));
...
function buildRocketNozzleElement(
  ...
  if (Math.abs(n.expansionEfficiency - 1) > EPSILON) {
    el.appendChild(
      elWithAttr(doc, 'ExpansionEfficiency', 'Value', formatG6(n.expansionEfficiency)),
    );
  }
