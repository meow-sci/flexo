...
| `EvaDoor`                  | `EVADoorTemplate.cs`                                                                | `<EVADoor>`                                                                         | attr `ConnectorId`; attr `SeatId` (5117) → `EvaDoor.seatId`, emitted only when set. ⚠️ `ConnectorId` has **no backing field** on `EVADoorTemplate` (5056 and 5117 both) — flexo emits it anyway; .NET's `XmlSerializer` ignores unknown attributes, so it is inert, but it is a flexo-only invention, not KSA schema                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | unchanged                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
⚠️ **Noted while closing Q1, NOT fixed:** flexo emits `<EVADoor ConnectorId="…">`, but
`EVADoorTemplate` has **no `ConnectorId` field** at 5117 _or_ at 5056 (its 5117 `SeatId` is
described in the decomp as the class's first field ever). `XmlSerializer` ignores unknown
attributes, so the attribute is inert rather than fatal, and the editor still uses
`EvaDoor.connectorId` internally to say which hatch geometry the door is. Removing it from the
export is a separate, deliberate change (it touches `types.ts`, the codec and the coupling UI)
and would alter export bytes, so it is left as-is and recorded here.
