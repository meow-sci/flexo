...
import { Button, SectionTitle, Switch, ToggleButton, ToggleButtonGroup } from '../kit';
...
/**
 * The active reference container's editor, rehosted from v1's left-centre floating card into
 * the ONE left-sidebar focus slot (design: design-build-mode.md §3.9). Guts unchanged.
 *
 * Its **gizmo mode** is deliberately independent of the Tool bar's Move/Rotate/Scale: a
 * container is an editor aide, not a document entity, and its scale re-normalizes through
 * `normalizeSize` (cylinders stay circular, spheres uniform) rather than writing free axes.
 *
 * **Undo enrollment**: every edit is discrete through `registerEditorAidStores`, pushed once
 * per typing session by `onInteractionStart`.
 */
...
export function ContainerEditorCard({ container: c }: { container: ReferenceContainer }) {
  ...
  return (
    <>
      ...
      <Button size="sm" variant="danger" onPress={() => removeContainer(c.id)}>
        Delete
      </Button>
    </>
  );
